...
set_bag_values_column = [ 
    [sg.Combo([st.SELECT_ISLAND_NAME_PRMOPT] + island_list, key = st.SELECT_ISLAND_NAME_KEY, default_value=st.SELECT_ISLAND_NAME_PRMOPT), sg.Button(f"{st.SELECT}", key = st.ISLAND_SELECTED_BUTTON_KEY)], 
    [sg.Text(f"{st.SELECTED_HEADER_PREFIX}"),
     sg.Text(f"{st.BLANK}", key = st.SELECTED_ISLAND_NAME_DISPLAY_KEY)
    ], 
    [buy_form_frame],
...
ship_column_capacity = [
    [ # first "row"
            ...
            [ # second "row"
            sg.Input(default_text = f"{world.ship.capacity}", enable_events=True, key = st.SHIP_CAPASITY_INPUT, size=(10,1) , expand_x=True , justification=st.CENTER)
            ]
            ]
...
ship_column_travlepoints= [
            [ # first "row"
            sg.Text(f"{st.TRAVEL_POINTS}:", expand_x=True , justification=st.CENTER)
            ],
            [ # second "row"
                sg.Input(f"{world.ship.travel_points}", enable_events=True, key = st.TRAVEL_POINTS_INPUT, size=(5,1), expand_x=True , justification=st.CENTER), 
            ]
            ]
...
ship_frame = sg.Frame(title=st.SHIP, layout= ship_frame_layout, expand_x=True)

mode_row = [sg.T(f"{st.MODE_HEADER}")] + [sg.Radio(mode_, group_id=st.WORLD_MODE, key = st.WORLD_MODE_KEY+ mode_, enable_events = True, default = mode_ == world.mode) for mode_ in world.POSIBLE_MODES]

ship_column = [
    [ship_frame],
    [sg.Text(f"{st.LOCATION}:" ), sg.Text(f"{world.ship.current_location.name}" , key = st.LOCATION_KEY )  ],    
    [sg.Combo([st.TRAVEL_ISLAND_NAME_PRMOPT] + island_list, key = st.TRAVEL_ISLAND_NAME_KEY, default_value=st.TRAVEL_ISLAND_NAME_PRMOPT), sg.Button(f"{st.TRAVEL}", key = st.TRAVEL_ISLAND_BUTTON_KEY)], 
    [sg.Text(f"{st.CONNECTIONS}:", expand_x=True , justification=st.CENTER)], 
    [sg.Table(
...
window_layout = [
    [sg.Menu(menu_def),
        header_row, 
        [   sg.Column(set_bag_values_column),
            sg.VSeparator(),
            sg.TabGroup([
                [sg.Tab(st.TAB_1_TITLE, best_1_out_island_tab),
                sg.Tab(st.TAB_2_TITLE, best_long_path_island_tab)]]),
            sg.VSeparator(),
            sg.Column(ship_column)],
    footer_row]
]

# window display trigger
window = sg.Window(st.WELCOME_STRING, window_layout, finalize=True, right_click_menu=right_click_menu) 
